 ...
 * 
 */
public class IWebFileCreating extends IApiCreatingFile {
	...
	public IWebFileCreating(Configuration configuration) {
		super(configuration);
	}

	public IWebFileCreating(CreateIApiFileConfig apiFileConfig) {
	...
	public void createFile() {
		// 生成文件
		super.createFile(this.apiFileConfig.getWebServicePackage(), this.apiFileConfig.gettWebServiceFileName(),
				content.toString());
	}
	...
	public void analysisApiFile() {
		...
		try(Reader reader = Resources.getResourceAsReader("imitate/mybatis/code/generation/mapping/xml/IWeb.xml")){
			apiBuilder = new XMLApiBuilder(this.configuration,reader);
			this.apiFileConfig = apiBuilder.parse();
			log.debug("解析web 层xml完成");
		} catch (IOException e) {
