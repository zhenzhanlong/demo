 ...
 * 
 */
public class IServiceFileCreating extends IApiCreatingFile {
	...
	public IServiceFileCreating(Configuration configuration) {
		super(configuration);
	}

	public IServiceFileCreating(CreateIApiFileConfig apiFileConfig) {
	...
	public void createFile() {
		// 生成文件
		super.createFile(this.apiFileConfig.getServicePackage(), this.apiFileConfig.getServiceFileName(),
				content.toString());
	}
	...
	public void analysisProperties() {
		...
		// 方法名称的配置关系加入集合
		pro.putAll(this.configuration.getServiceMethod());
	}
	...
	public void analysisApiFile() {
		...
		try (Reader reader = Resources
				.getResourceAsReader("imitate/mybatis/code/generation/mapping/xml/IService.xml")) {
			apiBuilder = new XMLApiBuilder(this.configuration,reader);
			this.apiFileConfig = apiBuilder.parse();
			log.debug("解析service 层xml完成");
		} catch (IOException e) {
